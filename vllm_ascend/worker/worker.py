...
class NPUWorker(WorkerBase):
    ...
    def initialize_from_config(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        with context:
            ...
            #
            # When EAGLE3 or MTP is enabled with num_speculative_tokens>1,
            # mamba blocks may be reallocated to full blocks if the target and
            # draft models share the same kv cache tensor (e.g. unaligned full
            # attn layers with different num_kv_heads and head_size). In
            # addition, for performance reasons, the current mtp/eagle path
            # does not update seq_lens_cpu with num_rejected_tokens for step>1,
            # since it would require d2h sync. As a result, seq_lens_cpu can
            # become stale and some blocks will be unintentionally used.
            #
            # If an uncleared mamba block is later reused, the stale state
            # combined with the incorrect seq_lens_cpu may lead to NaNs and
            ...
            if (
                ...
                and self.vllm_config.speculative_config.method in ("eagle3", "mtp")
