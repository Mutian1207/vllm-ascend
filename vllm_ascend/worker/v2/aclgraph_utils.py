...
class ModelAclGraphManager(ModelCudaGraphManager):
    ...
    def __init__(
        ...
        trace_log(
            "flow.graph.manager",
            "Ascend ACL graph manager is active: mode=%s capture_sizes=%s",
            cudagraph_mode,
            self.capture_sizes,
        )
    ...
    def run_fullgraph(self, desc: BatchExecutionDescriptor) -> torch.Tensor | tuple[torch.Tensor, list[torch.Tensor]]:
        ...
        trace_log(
            "flow.graph.run",
            "running captured full graph for this model step: num_tokens=%s "
            "num_reqs=%s",
            num_tokens,
            desc.num_reqs,
        )
    ...
    def capture(
        ...
        trace_log("flow.graph.capture", "ACL graph capture finished")
        return ret
