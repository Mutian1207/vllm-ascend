...
- **Location**: `vllm_ascend/worker/v2/block_table.py` —
...
- **Algorithm flow**:
  ...
  3. Process the interval in `TRITON_BLOCK_SIZE` tiles. Convert positions to
     INT32, calculate block indices and offsets using the upstream
     `block_size * CP_SIZE` expression, and load physical block numbers only
     for valid token lanes.
...
- **Differences**:
  - Converts logical positions from INT64 to INT32 before block-index,
    ...
    on Ascend NPU.
  - Masks the indirect block-table load to active token lanes, avoiding address
    calculation and memory access for lanes outside the current request.
  - Otherwise preserves the upstream direct block-table addressing, launch
    grid, CP mapping, padding behavior, and optional `out` semantics.
